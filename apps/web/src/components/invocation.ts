...
export class Invocation {
  ...
  constructor(
    ...
    const master = this.gain(voice.level)
  ...
  private sing(when: number) {
    ...
    const undertoneLevel = this.gain(this.voice.undertone)
    ...
    const shaper = this.own(this.context.createWaveShaper())
    shaper.curve = saturationCurve(this.voice.rasp)
    drive.connect(shaper)
    ...
    const formants = weights.map((weight, i) => {
      const formant = this.filter(
        'bandpass',
        [500, 1500, 2500, 2900][i] * this.voice.tract,
        qs[i],
      )
    ...
    // diction legible.
    const gritDrive = this.gain(this.voice.gritDrive)
    const gritShaper = this.own(this.context.createWaveShaper())
    gritShaper.curve = saturationCurve(5)
    const gritDark = this.filter('lowpass', this.voice.gritDark, 0.7)
    const gritLevel = this.gain(this.voice.grit)
    ...
    const patina = this.filter('lowpass', this.voice.patina, 0.6)
